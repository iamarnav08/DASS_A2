...
def initialize_pieces():
    """Function to make the pieces on the gameboard upon 
...
def make_grid(board, pieces, font):
    """
    A function to draw the grid for the game.
    """
    for i in range(4):
...
def win_condition(board, font):
    """
    What to do if a player wins
    """
    if board.winner:
...
def piece_move_handler(board, pieces, x, y, selected_piece):
    """
    Function to handle movement of pieces when game is running
    """
    if selected_piece:
        cell = board.get_grid_sq(x,y)
        if cell and board.check_availability(cell[0], cell[1], selected_piece):
            if any(selected_piece in cell_pieces for row in board.cells for cell_pieces in row):
...
def main():
    ...
    while True:
        ...
        # Making the grid
        make_grid(board, pieces, font)

        # Checking for winner
        win_condition(board, font)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                selected_piece=piece_move_handler(board, pieces, x, y, selected_piece)
