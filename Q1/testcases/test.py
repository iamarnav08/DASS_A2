...
from app.routes.auth_control import login_user, register_user
from app.routes.customer_options import view_profile as customer_view_profile
from app.routes.restaurant_options import view_profile as restaurant_view_profile, view_received_orders
from app.routes.manager_options import manage_delivery_fleet, view_restaurant_pov
from app.routes.delivery_agent_options import view_delivery_details
...
class TestFoodDelivery(unittest.TestCase):
    ...
    def test_valid_login_delivery_agent(self):
        # Don't create a new agent - use the one from setUp
        with patch('builtins.input', side_effect=['2', self.test_agent.phone_number, "testpass"]):
            # Patch delivery_agent_options to prevent menu display during test
            with patch('app.routes.delivery_agent_options.delivery_agent_options', return_value=None):
                with redirect_stdout(StringIO()) as stdout:
                    result = login_user()
                    output = stdout.getvalue()
                
                self.assertIsInstance(result, DeliveryAgent)
                self.assertIn("Delivery Agent logged in successfully", output)
    ...
    def test_no_restaurants_available_while_ordering(self):
        ...
        with patch('builtins.input', side_effect=['1']):
            with self.assertRaises(Exception) as context:
                from app.routes.customer_options import place_orders
